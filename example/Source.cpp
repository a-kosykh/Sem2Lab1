	...
	case WM_COMMAND:
		...
		case IDC_BUTTON5:
			SendDlgItemMessage(hwnd, IDC_EDIT1, WM_GETTEXT, 63, (LPARAM)Text); 
				...
				{
					MessageBox(hwnd, "ERROR: First input",
						"Wrong value", MB_OK | MB_ICONHAND);
					return TRUE;
				}
			SendDlgItemMessage(hwnd, IDC_EDIT2, WM_GETTEXT, 63, (LPARAM)Text); 
				if (sscanf_s(Text, "%lf", &y)<1)
				{
					MessageBox(hwnd, "ERROR: Second input",
						"Wrong value", MB_OK | MB_ICONHAND);
					return TRUE;
				}
			char Znak;
			...
			if (LOWORD(wParam) == IDC_BUTTON4) { z = x * y; Znak = '*'; }
			if (LOWORD(wParam) == IDC_BUTTON5) { z = x / y; Znak = '/'; }
			sprintf(Text, "%f", z);
